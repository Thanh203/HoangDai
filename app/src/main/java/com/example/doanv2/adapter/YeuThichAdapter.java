...
public class YeuThichAdapter extends RecyclerView.Adapter<YeuThichAdapter.YeuThichVH>{
    ArrayList<MonAnDonGian> cacMonAn;
    Context context;
    ItemCallback itemCallback;

    public YeuThichAdapter(ArrayList<MonAnDonGian> cacMonAn, Context context, ItemCallback itemCallback) {
    ...
    public YeuThichVH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        context = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(context);

        View view = inflater.inflate(R.layout.layoutitem2, parent, false);
        return new YeuThichVH(view);
    }

    @Override
    public void onBindViewHolder(@NonNull YeuThichVH holder, int position) {
    ...
    @NonNull
    private View.OnClickListener FavoriteClick(@NonNull YeuThichVH holder, MonAnDonGian monAn, MonAnDB monAnDB) {
    ...
    class YeuThichVH extends RecyclerView.ViewHolder{
        ImageView Anh;
        TextView TenMonAn;
        ImageButton ibLYeuThich;
        public YeuThichVH(@NonNull View itemview){
            super(itemview);
